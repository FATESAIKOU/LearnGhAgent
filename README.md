...
4. **執行**：組合 issue 內容 + 角色 instructions + workflow context 為 prompt，呼叫 `gh copilot -p "..." --yolo --no-ask-user --output-format json`
5. **回寫**：將 Agent 輸出作為 comment 回寫到 Issue
6. **階段轉換**：移除當前 `role:` + `phase:` label，加上下一階段的 label（若有 Workflow），下次輪詢時自動執行下一階段
