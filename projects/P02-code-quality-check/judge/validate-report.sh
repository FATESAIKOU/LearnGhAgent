...
content="$(cat "$file")"
if (( ${#content} > 10000 )); then echo "FAIL: $file length ${#content} > 10000" >&2; exit 1; fi
...
if ! [[ "$basename" =~ ^[0-9]+_.+\.md$ ]]; then
  echo "FAIL: report filename '$basename' does not match (pr-id)_(round-id)_quality-report.md" >&2; exit 1
fi
